...
<head>
    ...
    <title>Edit Job - {{ $job->name }} - Company Dashboard</title>
        ...
        <div class="container">
            <div class="row">
                <div class="col-12 text-center">
                    <h1 class="fw-bold mb-3">Edit Job</h1>
                    <p class="lead mb-0">Update your job posting</p>
                </div>
            </div>
        </div>
    </section>
    ...
    <section class="py-5" style="background: #f8f9fa;">
        <div class="container">
            <div class="form-container">
                ...
                <form action="{{ route('superuser.jobs.update', $job->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-4 mb-3">
                                ...
                                <select class="form-select @error('employment_type') is-invalid @enderror" 
                                    ...
                                    <option value="Remote" {{ old('employment_type', $job->employment_type) == 'Remote' ? 'selected' : '' }}>Remote</option>
                                    <option value="Hybrid" {{ old('employment_type', $job->employment_type) == 'Hybrid' ? 'selected' : '' }}>Hybrid</option>
                                </select>
                    ...
                    <div class="form-section">
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control @error('description') is-invalid @enderror" 
                                      ...
                                      placeholder="Describe the role, responsibilities, and what the candidate will be doing..." required>{{ old('description', $job->description) }}</textarea>
                            @error('description')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control @error('requirements') is-invalid @enderror" 
                                      ...
                                      placeholder="List the qualifications, skills, and experience required..." required>{{ old('requirements', $job->requirements) }}</textarea>
                            @error('requirements')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        @if($categories->count() > 0)
                            <div class="checkbox-group">
                                @foreach($categories as $category)
                                    @php
                                        $isChecked = in_array($category->id, old('categories', $job->categories->pluck('id')->toArray()));
                                    @endphp
